...
class ServerState {
  ...
  final String selectedType; // 'All', 'Servers', 'EME Profile'
  ...
  List<ServerModel> get joinedServers {
    return servers
        .where((s) => s.isJoined && s.providerType == ProviderType.server)
        .toList();
  }
  ...
  int get serverCount =>
      servers.where((s) => s.providerType == ProviderType.server).length;

  int get individualCount =>
      servers.where((s) => s.providerType == ProviderType.individual).length;

  /// Individuals / EME Profile filtered for EME World
  ...
  List<ServerModel> get filteredCatalog {
    return servers.where((item) {
      // Type Filter
      if (selectedType == 'Servers' &&
          item.providerType != ProviderType.server) {
        return false;
      }
      if ((selectedType == 'EME Profile' || selectedType == 'Individuals') &&
    ...
    : super(
        ServerState(
          servers: [
            ...
            // ==========================================
            // INDIVIDUAL SERVICE PROVIDERS & EME Profile
            // ==========================================
            const ServerModel(
              id: 'ind_001',
              title: 'Dr. Maya Lin',
