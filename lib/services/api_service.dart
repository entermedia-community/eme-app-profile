...
abstract class IApiService {
  ...
  Future<List<ProductMessageModel>> fetchProducts({String? serverId, ProductType? type});
...
class ApiService implements IApiService {
  ...
  Future<dynamic> _get(String path, {Map<String, dynamic>? mockFallback}) async {
  ...
  Future<List<EmeProfileModel>> fetchSpecialistProfiles() async {
    ...
    final list = res['data'] as List<dynamic>? ?? [];
    return list.map((item) => EmeProfileModel.fromJson(item as Map<String, dynamic>)).toList();
  }
  ...
  Future<ProfileModel> fetchUserProfile({String? userId}) async {
    final mock = {
      'data': {
        ...
        'bio': 'Pioneering regenerative mesh networks, peer-to-peer commerce, and decentralized intelligence.',
  ...
  Future<List<ProductMessageModel>> fetchProducts({
    ...
    final list = res['data'] as List<dynamic>? ?? [];
    var products = list
        .map((item) => ProductMessageModel.fromJson(item as Map<String, dynamic>))
        .toList();
  ...
  Future<List<ChatModel>> fetchChats() async {
    ...
    final list = res['data'] as List<dynamic>? ?? [];
    return list.map((item) => ChatModel.fromJson(item as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<GoalItemModel>> fetchServerGoals(String serverId) async {
    ...
    final list = res['data'] as List<dynamic>? ?? [];
    return list.map((item) => GoalItemModel.fromJson(item as Map<String, dynamic>)).toList();
  }
  ...
  @override
  Future<List<TransactionItemModel>> fetchServerTransactions(String serverId) async {
    final mock = {
      'data': [
        {
          'id': 'tx_01',
    ...
    final res = await _get('/servers/$serverId/transactions', mockFallback: mock);
    final list = res['data'] as List<dynamic>? ?? [];
    return list.map((item) => TransactionItemModel.fromJson(item as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<BlogPostModel>> fetchServerBlogPosts(String serverId) async {
    ...
    final list = res['data'] as List<dynamic>? ?? [];
    return list.map((item) => BlogPostModel.fromJson(item as Map<String, dynamic>)).toList();
  }
}
